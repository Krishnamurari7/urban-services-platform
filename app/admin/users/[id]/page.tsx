...
export default async function UserDetailPage({
  ...
  const { profile, email, customerBookings, professionalBookings, addresses, reviewsGiven, reviewsReceived } = data;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            {profile.role === "customer" ? "Customer" : profile.role === "professional" ? "Professional" : "Admin"} Profile
          </p>
        </div>
        <Link href="/admin/users">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
                  ...
                  <>
                    {profile.rating_average && (
                      <div>
                        ...
                        <p className="mt-1">
                          {profile.rating_average.toFixed(1)} ⭐ ({profile.total_reviews || 0} reviews)
                        </p>
                      </div>
                    )}
              ...
              {isProfessional && profile.skills && profile.skills.length > 0 && (
                <div>
          ...
          {isCustomer && customerBookings.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle>Recent Bookings ({data.totalCustomerBookings} total)</CardTitle>
              </CardHeader>
              <CardContent>
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-gray-500">
                            {booking.professional?.full_name} • {booking.service?.category}
                          </p>
          ...
          {isProfessional && professionalBookings.length > 0 && (
            <Card>
              <CardHeader>
                <CardTitle>Recent Jobs ({data.totalProfessionalBookings} total)</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="font-medium">{booking.service?.name}</p>
                          <p className="text-sm text-gray-500">
                            {booking.customer?.full_name} • {booking.service?.category}
                          </p>
                          <p className="text-xs text-gray-400 mt-1">
                            {new Date(booking.scheduled_at).toLocaleString()}
                          </p>
              ...
              {isCustomer && (
                <>
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{data.totalCustomerBookings}</span>
                  </div>
                  <div className="flex justify-between">
              ...
              {isProfessional && (
                <>
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{data.totalProfessionalBookings}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-600">Rating</span>
                    <span className="font-medium">
                      {profile.rating_average ? `${profile.rating_average.toFixed(1)} ⭐` : "N/A"}
                    </span>
                  </div>
                  <div className="flex justify-between">
